...
function [respMat ] = presentSocialChoiceTrainV5(window,scr_rect,...
    ...
    lFaceType,rFaceType,rankVis,curTexts,blockTime,delayOff,choiceOff,feedbackOff,trustOff)
...
%   end
 ...
 %%%%% SHOW CHOICE, RECORD RESPONSE
    Screen('DrawTexture', window, faces(lFaceType), source_rect, box2);
    Screen('DrawTexture', window, faces(rFaceType), source_rect, box3); 
...
if isnan(respMat(1))
   ...
   respMat(8)=NaN; % Face stimulus picked
    ...
    rank=round(rVec(faceTypes(respMat(1)))+curRNoise); % rank feedback
    if rank < 1
        rank=1;
    end
    if rank > 8
    ...
    size=round(gVec(faceTypes(respMat(1)))+curGNoise);  % group size
    if size < 1
        size = 1;
    end
    if size > 8
    ...
    outcome=(rank<=size)*1; 
    curRankType=rVec(faceTypes(respMat(1)));
    curGroupType=gVec(faceTypes(respMat(1)));
 ...
 %%   oppTrait=gVec(3-context,faceTypes(respMat(1)));
     respMat(7)=faceTypes(respMat(1)); % Which face type picked
     respMat(8)=faceAssign(faceTypes(respMat(1))); % Face Stimulus picked
...
Screen('TextSize', window, 22);
    ...
    Screen('DrawTexture', window, faces(rFaceType), source_rect, box3);
        ...
        curBox=eval(boxes{i});
      Screen('DrawTexture', window, faces(faceTypes(respMat(1))), source_rect, box5); %display who you picked
...
Screen('Flip', window);
 ...
 %%%%%% TRUST GAME
   ...
   elseif outcome==0
    ...
    Screen('DrawTexture', window, faces(faceTypes(respMat(1))), source_rect, box5);
